        ...
        if (!userId && isLocalhost) {
            ...
            if (editingNoteId) {
                ...
                if (noteIndex !== -1) {
                    ...
                    elements.statusDiv.textContent = '✅ Catatan berhasil diperbarui! (Mode Development)';
                } else {
                    elements.statusDiv.textContent = '❌ Catatan tidak ditemukan untuk diperbarui';
                    return;
                }
            } else {
                // Add new note
                localNotes.unshift(noteData);
                elements.statusDiv.textContent = '✅ Catatan berhasil disimpan! (Mode Development)';
            }
...
window.deleteNote = async (noteId) => {
    ...
    try {
        ...
        if (isLocalhost && !userId) {
            ...
            elements.statusDiv.textContent = '✅ Catatan berhasil dihapus (Mode Development)';
...
const initializeAuth = () => {
    ...
    if (isLocalhost) {
        ...
        elements.statusDiv.textContent = '🧪 Mode Development - Data disimpan lokal';
